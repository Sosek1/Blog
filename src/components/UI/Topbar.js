...
const Topbar = () => {
    ...
    <header className="h-[60px] bg-light-900 ml-[20px] mr-[20px] flex items-center justify-between">
        ...
        {!loginCtx.isLoggedIn && loginCtx.loginScreen && <div className="flex flex-row">
            <p className="font-sans text-h4-mobile font-h3-mobile text-dark/100" onClick={loginCtx.onLogout}>Go back</p>
        </div>}
        {loginCtx.isLoggedIn && !loginCtx.loginScreen && <div className="flex flex-row">
            <p className="font-sans text-h4-mobile font-h3-mobile text-dark/100">Log out</p>
        </div>}
    </header>
    </>
    )
